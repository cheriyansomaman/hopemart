...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setStockMode(data.stockType as StockMode || 'quantity');

        if (data.stockType === 'weight') {
          setTotalWeightKg(data.totalWeight?.toString() || '');
        } else {
          setStock(data.stock?.toString() || '');
          if ((data as unknown as ProductData).weightPerUnit) {
            setShowWeight(true);
            setWeightPerUnit(((data as unknown as ProductData).weightPerUnit ?? 0).toString());
          }
          if ((data as unknown as ProductData).dimensions) {
            const dims = (data as unknown as ProductData).dimensions;
            setShowDimensions(true);
            setDimWidth(dims?.width?.toString() || '');
            setDimHeight(dims?.height?.toString() || '');
            setDimLength(dims?.length?.toString() || '');
          }
        }

        const savedImage = data.productImage || data.imageUrl || '';
        if (savedImage) {
          setCurrentProductImage(savedImage);
          setImagePreview(savedImage);
        }
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        setError('Failed to load product data: ' + msg);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.size > MAX_PRODUCT_IMAGE_BYTES) {
      ...
      setImage(null);
      setImagePreview(currentProductImage);
      e.target.value = '';
      return;
    }
    setError('');
    setImage(file);
    setImagePreview(file ? URL.createObjectURL(file) : currentProductImage);
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (stockMode === 'quantity' && !stock) {
      setError('Stock quantity is required.');
      return;
    }
    if (stockMode === 'weight' && !totalWeightKg) {
    ...
    setSaving(true);
    try {
      let productImage = currentProductImage;
      if (image) {
      ...
      if (stockMode === 'quantity') {
        fields.stock = parseInt(stock, 10);
        fields.weightPerUnit = showWeight && weightPerUnit ? parseFloat(weightPerUnit) : null;
        fields.dimensions = showDimensions && (dimWidth || dimHeight || dimLength)
          ? {
          ...
          : null;
        fields.totalWeight = null;
        fields.weightUnit = null;
      } else {
        fields.stock = 0;
        fields.totalWeight = parseFloat(totalWeightKg);
        fields.weightUnit = 'kg';
        fields.weightPerUnit = null;
        fields.dimensions = null;
      }

      await productService.update(id, fields as Parameters<typeof productService.update>[1]);
