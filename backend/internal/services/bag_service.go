...
func (s *BagService) Get(ctx context.Context, userID string) ([]models.BagItem, error) {
	docs, err := s.repo.GetAll(ctx, s.repo.Collection("bags").Doc(userID).Collection("items").Query)
	if err != nil {
		return nil, err
	}

	items := make([]models.BagItem, 0, len(docs))
...
func (s *BagService) AddOrUpdate(ctx context.Context, userID string, item models.BagItem) error {
