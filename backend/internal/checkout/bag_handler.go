...
func NewBagHandler(svc *BagService) *BagHandler {
	return &BagHandler{svc: svc}
}

func (h *BagHandler) RegisterRoutes(r gin.IRouter) {
	r.GET("/bag", h.Get)
	r.POST("/bag", h.AddOrUpdate)
	r.DELETE("/bag/:itemId", h.Remove)
	r.DELETE("/bag", h.Clear)
	r.PATCH("/bag/:itemId/qty", h.UpdateQty)
}
